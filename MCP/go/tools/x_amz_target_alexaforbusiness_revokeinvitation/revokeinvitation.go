...
func RevokeinvitationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.RevokeInvitationRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=AlexaForBusiness.RevokeInvitation", cfg.BaseURL)
		...
		var result models.RevokeInvitationResponse
...
func CreateRevokeinvitationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AlexaForBusiness_RevokeInvitation",
		mcp.WithDescription("Revokes an invitation and invalidates the enrollment URL."),
		...
		mcp.WithString("EnrollmentId", mcp.Description("")),
		mcp.WithString("UserArn", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    RevokeinvitationHandler(cfg),
	}
}
