...
func DisassociatedevicefromroomHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.DisassociateDeviceFromRoomRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=AlexaForBusiness.DisassociateDeviceFromRoom", cfg.BaseURL)
		...
		var result models.DisassociateDeviceFromRoomResponse
...
func CreateDisassociatedevicefromroomTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AlexaForBusiness_DisassociateDeviceFromRoom",
		mcp.WithDescription("Disassociates a device from its current room. The device continues to be connected to the Wi-Fi network and is still registered to the account. The device settings and skills are removed from the room."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DisassociatedevicefromroomHandler(cfg),
	}
}
