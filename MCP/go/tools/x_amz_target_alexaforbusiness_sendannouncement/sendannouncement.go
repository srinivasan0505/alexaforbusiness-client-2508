...
func SendannouncementHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.SendAnnouncementRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=AlexaForBusiness.SendAnnouncement", cfg.BaseURL)
		...
		var result models.SendAnnouncementResponse
...
func CreateSendannouncementTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AlexaForBusiness_SendAnnouncement",
		mcp.WithDescription("Triggers an asynchronous flow to send text, SSML, or audio announcements to rooms that are identified by a search or filter. "),
		...
		mcp.WithString("TimeToLiveInSeconds", mcp.Description("")),
		mcp.WithString("ClientRequestToken", mcp.Required(), mcp.Description("")),
		mcp.WithObject("Content", mcp.Required(), mcp.Description("")),
		mcp.WithString("RoomFilters", mcp.Required(), mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    SendannouncementHandler(cfg),
	}
}
