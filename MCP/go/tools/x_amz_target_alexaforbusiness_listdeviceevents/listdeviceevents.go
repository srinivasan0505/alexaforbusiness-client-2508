...
func ListdeviceeventsHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListDeviceEventsRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=AlexaForBusiness.ListDeviceEvents%s", cfg.BaseURL, queryString)
		...
		var result models.ListDeviceEventsResponse
...
func CreateListdeviceeventsTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AlexaForBusiness_ListDeviceEvents",
		mcp.WithDescription("Lists the device event history, including device connection status, for up to 30 days."),
		...
		mcp.WithString("EventType", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListdeviceeventsHandler(cfg),
	}
}
