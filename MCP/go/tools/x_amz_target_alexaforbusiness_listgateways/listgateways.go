...
func ListgatewaysHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListGatewaysRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=AlexaForBusiness.ListGateways%s", cfg.BaseURL, queryString)
		...
		var result models.ListGatewaysResponse
...
func CreateListgatewaysTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AlexaForBusiness_ListGateways",
		mcp.WithDescription("Retrieves a list of gateway summaries. Use GetGateway to retrieve details of a specific gateway. An optional gateway group ARN can be provided to only retrieve gateway summaries of gateways that are associated with that gateway group ARN."),
		...
		mcp.WithString("GatewayGroupArn", mcp.Description("")),
		mcp.WithString("MaxResults", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListgatewaysHandler(cfg),
	}
}
