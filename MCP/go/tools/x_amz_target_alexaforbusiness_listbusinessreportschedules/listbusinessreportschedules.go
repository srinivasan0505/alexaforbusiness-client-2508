...
func ListbusinessreportschedulesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.ListBusinessReportSchedulesRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=AlexaForBusiness.ListBusinessReportSchedules%s", cfg.BaseURL, queryString)
		...
		var result models.ListBusinessReportSchedulesResponse
...
func CreateListbusinessreportschedulesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AlexaForBusiness_ListBusinessReportSchedules",
		mcp.WithDescription("Lists the details of the schedules that a user configured. A download URL of the report associated with each schedule is returned every time this action is called. A new download URL is returned each time, and is valid for 24 hours."),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ListbusinessreportschedulesHandler(cfg),
	}
}
