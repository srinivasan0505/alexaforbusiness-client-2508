...
func SendinvitationHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		var requestBody models.SendInvitationRequest
		...
		url := fmt.Sprintf("%s/#X-Amz-Target=AlexaForBusiness.SendInvitation", cfg.BaseURL)
		...
		var result models.SendInvitationResponse
...
func CreateSendinvitationTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_#X-Amz-Target=AlexaForBusiness_SendInvitation",
		mcp.WithDescription("Sends an enrollment invitation email with a URL to a user. The URL is valid for 30 days or until you call this operation again, whichever comes first. "),
		...
		mcp.WithString("UserArn", mcp.Description("")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    SendinvitationHandler(cfg),
	}
}
